(function initPulsarTimelineRuntime() {
  "use strict";
  ...
  function timelineForGridCell(cell) {
    ...
    if (selectionAxis) {
      return {
        ...
        hasPlot: !!(cell.querySelector && cell.querySelector(".pulsar-plot1d")),
        hasSequence: !!(cell.querySelector && cell.querySelector('img[data-grid-image-sequence="1"]')),
        axis: selectionAxis,
      };
    }

    const plots = Array.from(cell.querySelectorAll ? cell.querySelectorAll(".pulsar-plot1d") : []);
  ...
  function selectedTimelineDriverCell() {
    const cell = gridRuntimeQuery('.pulsar-dropcell[data-timeline-driver="1"]');
    return cell && isVisibleGridCell(cell) ? cell : null;
  }

  function activeTimelineCell() {
    const cell = gridRuntimeQuery('.pulsar-dropcell[data-cell-active="1"]');
    return cell && isVisibleGridCell(cell) ? cell : null;
  }

  function autoTimelineDriver() {
    const cells = Array.from(gridRuntimeQueryAll(".pulsar-dropcell"));
    let best = null;
  ...
  function elementSelectionAxis(el) {
    const cell = el && el.closest ? el.closest(".pulsar-dropcell") : null;
  ...
  function updateAxisCompatibility(activeAxis) {
    for (const cell of Array.from(gridRuntimeQueryAll(".pulsar-dropcell"))) {
  ...
  function getVcrSliderFrameValue() {
    const slider = gridRuntimeElementById("pulsar-vcr-step-slider");
  ...
  function updateVcrTimeLabelFromSeconds(rawSeconds, videos, plots) {
    ...
    const label = gridRuntimeElementById("pulsar-vcr-time-value");
  ...
  function getGridPlots() {
    const plot = plotRuntime();
    return plot ? plot.getPlots() : Array.from(gridRuntimeQueryAll(".pulsar-plot1d"));
  }
    ...
    if (mutation.type === "attributes") {
      ...
      return !!(
        ...
        && (
          ...
          || target.matches(".pulsar-plot1d")
          || target.matches(".pulsar-dropcell")
        )
      );
    }
    for (const node of Array.from(mutation.addedNodes || [])) {
      ...
      if (
        (node.matches && (
          ...
          || node.matches(".pulsar-plot1d")
          || node.matches(".pulsar-dropcell")
        ))
        || (node.querySelector && node.querySelector(
          'img[data-grid-image-sequence="1"], video[data-grid-video="1"], .pulsar-plot1d, .pulsar-dropcell'
        ))
      ) {
        return true;
      }
  ...
  function onGridRuntimeSlider(e) {
    ...
    if (target && target.id === "pulsar-vcr-step-slider") {
  ...
  function plotRuntime() {
    ...
    const runtimes = pulsar.runtimes || {};
  ...
  function unmountTimelineRuntime(root) {
    ...
    stopSyncTimer();
    root.removeAttribute("data-pulsar-timeline-runtime-owner");
    runtimeRoot = null;
  }
  ...
  function mountTimelineRuntime(root) {
    ...
    runtimeRoot = root;
    root.setAttribute("data-pulsar-timeline-runtime-owner", "mounted");
    updateGridVcrToggleButton();
  ...
  window.pulsarTimelineRuntime = runtime;
})();
