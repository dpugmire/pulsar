...
from pulsar.models import canvas_layout
from pulsar.widgets import GridRuntime
...
def _build_grid_size_picker(ctrl):
    with html.Div(classes="pulsar-toolbar-menu", style="margin-top:10px; width:100%;"):
        with html.Button(
            classes="pulsar-grid-size-trigger",
            raw_attrs=['type="button"'],
            title="Grid size",
        ):
            html.Span("Grid size", classes="pulsar-grid-size-trigger-label")
            html.Span("{{ gridRows + ' x ' + gridCols + ' ▾' }}", classes="pulsar-grid-size-trigger-value")
        with vuetify.VMenu(
            activator="parent",
            location="bottom start",
            close_on_content_click=False,
        ):
            with vuetify.VCard(classes="pulsar-grid-size-popover", elevation=4):
                with vuetify.VCardText(class_="pa-2"):
                    html.Div("Grid size", classes="pulsar-toolbar-popover-title")
                    with html.Div(classes="pulsar-grid-picker"):
                        for picker_row in range(1, 9):
                            for picker_col in range(1, 9):
                                html.Button(
                                    "",
                                    classes="pulsar-grid-picker-cell",
                    ...
                    html.Div(
                        "{{ gridRows + ' x ' + gridCols }}",
                        class_="text-caption pulsar-grid-picker-label",
                    )
                    with html.Div(classes="pulsar-grid-layout-stepper"):
                        html.Span("Rows", class_="text-caption pulsar-grid-layout-stepper-label")
                        html.Button(
                            "-",
                            classes="pulsar-grid-layout-btn",
                    ...
                    with html.Div(classes="pulsar-grid-layout-stepper"):
                        html.Span("Cols", class_="text-caption pulsar-grid-layout-stepper-label")
                        html.Button(
                            "-",
                            classes="pulsar-grid-layout-btn",
...
def _build_canvas_zoom_controls(ctrl):
    with vuetify.Template(v_if="gridLayoutMode === 'freeform'"):
        with html.Div(
            classes="pulsar-canvas-zoom-controls",
            raw_attrs=['aria-label="Canvas zoom controls"'],
        ):
            html.Button(
                "−",
                classes="pulsar-canvas-zoom-button",
            ...
            html.Span(
                ...
                classes="pulsar-canvas-zoom-value",
            ...
            html.Button(
                "Fit",
                classes="pulsar-canvas-fit-button",
...
def _build_grid_settings_popover(ctrl):
    html.Div("Settings", classes="pulsar-toolbar-popover-title")
    with html.Div(classes="pulsar-settings-section"):
        html.Div("Layout", classes="pulsar-settings-section-title")
        html.Div("Cell layout", classes="pulsar-grid-sizing-section-label")
        with html.Div(classes="pulsar-grid-sizing-mode pulsar-layout-mode-selector"):
            html.Button(
                "Uniform",
                classes="pulsar-grid-sizing-mode-btn",
        ...
        with vuetify.Template(v_if="gridLayoutMode === 'freeform'"):
            html.Div("Canvas behavior", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-canvas-settings"):
                html.Button(
                    "Snap to grid",
                    classes="pulsar-canvas-toggle",
            ...
            html.Div("New plot size", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-size-stepper"):
                html.Button(
                    "−",
                    classes="pulsar-grid-layout-btn",
                ...
                html.Span(
                    ...
                    classes="pulsar-size-stepper-value pulsar-canvas-default-size-value",
                )
                html.Button(
                    "+",
                    classes="pulsar-grid-layout-btn",
            ...
            html.Div(
                ...
                classes="pulsar-canvas-settings-hint",
            )
            html.Div("Grid columns", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-canvas-column-options"):
                for columns in canvas_layout.CANVAS_COLUMN_CHOICES:
                    html.Button(
                        str(columns),
                        classes="pulsar-canvas-column-option",
        ...
        with vuetify.Template(v_if="gridLayoutMode !== 'freeform'"):
            html.Div("Size mode", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-grid-sizing-mode"):
                html.Button(
                    "Static",
                    classes="pulsar-grid-sizing-mode-btn",
        ...
        with vuetify.Template(v_if="gridLayoutMode !== 'freeform' && gridSizingMode !== 'fit'"):
            html.Div("Cell size", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-size-stepper"):
                html.Button(
                    "-",
                    classes="pulsar-grid-layout-btn",
                ...
                html.Input(
                    ...
                    classes="pulsar-size-stepper-input",
                ...
                html.Span("px", classes="pulsar-size-stepper-unit")
            html.Button(
                "Reset track sizes",
                classes="pulsar-grid-reset-tracks-btn",
                click=ctrl.reset_grid_track_sizes,
                raw_attrs=['type="button"'],
                title="Reset all rows and columns to the current cell size",
            )
        with vuetify.Template(v_if="gridLayoutMode !== 'freeform' && gridSizingMode === 'fit'"):
            html.Div("Minimum cell size", classes="pulsar-grid-sizing-section-label")
            with html.Div(classes="pulsar-size-stepper"):
                html.Button(
                    "-",
                    classes="pulsar-grid-layout-btn",
    ...
    with html.Div(classes="pulsar-settings-section"):
        html.Div("Scalar plots", classes="pulsar-settings-section-title")
        with html.Div(classes="pulsar-settings-row"):
...
def _build_grid_layout_controls(ctrl):
    with html.Div(classes="pulsar-grid-layout-controls"):
        with html.Div(classes="pulsar-toolbar-menu"):
            html.Button(
                "⚙",
                classes="pulsar-toolbar-menu-btn pulsar-toolbar-icon-btn",
                raw_attrs=[
                    'type="button"',
                    'aria-label="Settings"',
                ],
            ...
            with vuetify.VMenu(
                ...
                with vuetify.VCard(classes="pulsar-settings-popover", elevation=4):
                    with vuetify.VCardText(class_="pa-2"):
...
def _build_workspace_tab_bars(ctrl):
    with vuetify.Template(v_for="(pane, paneIndex) in workspacePanes", key="pane.id"):
        with html.Div(
            classes="pulsar-workspace-tab-bar",
            raw_attrs=[
                ":class=\"[paneIndex === 0 ? 'pulsar-workspace-slot-first' : 'pulsar-workspace-slot-second', pane.id === workspaceActivePaneId ? 'is-active-pane' : '']\"",
            ...
            with html.Div(classes="pulsar-workspace-tabs-viewport"):
                with html.Div(
                    classes="pulsar-workspace-tabs",
                    role="tablist",
                    raw_attrs=[':data-pane-id="pane.id"'],
                ):
                    with vuetify.Template(v_for="tab in pane.tabs", key="tab.id"):
                        with html.Div(
                            classes="pulsar-workspace-tab-shell",
                            raw_attrs=[
                            ...
                            html.Button(
                                ...
                                classes="pulsar-workspace-tab-close",
                                click=(
                    ...
                    html.Button(
                        "+",
                        classes="pulsar-workspace-tab-add",
        ...
        with vuetify.Template(v_if="pane.id !== workspaceActivePaneId"):
            with vuetify.Template(v_for="tab in pane.tabs", key="tab.id"):
                with vuetify.Template(v_if="tab.id === pane.active_tab_id"):
                    with html.Div(
                        classes="pulsar-main-grid pulsar-workspace-grid-preview",
                        click=(ctrl.activate_workspace_tab, "[pane.id, tab.id]"),
                        raw_attrs=[
                            ":class=\"[paneIndex === 0 ? 'pulsar-workspace-slot-first' : 'pulsar-workspace-slot-second', { 'pulsar-freeform-preview': tab.grid.layout_mode === 'freeform' }]\"",
                            ':data-pane-frame-id="pane.id"',
                            ':data-pane-id="pane.id"',
                            ':data-tab-id="tab.id"',
                        ...
                        with html.Div(
                            classes="pulsar-canvas-grid-overlay",
                            ...
                            with vuetify.Template(
                                ...
                                html.Div(
                                    classes="pulsar-canvas-grid-line is-vertical",
                                    raw_attrs=[
                            ...
                            with vuetify.Template(
                                ...
                                html.Div(
                                    classes="pulsar-canvas-grid-line is-horizontal",
                                    raw_attrs=[
                                        ':class="{ \'is-major\': line % 4 === 0 }"',
                                        ':data-grid-line="line"',
                        ...
                        html.Div(
                            classes="pulsar-canvas-placeholder pulsar-canvas-preview-placeholder",
                        ...
                        with vuetify.Template(
                            ...
                            with html.Div(
                                classes="pulsar-dropcell pulsar-workspace-preview-cell",
                                raw_attrs=[
                                    ':data-cell-index="i"',
                                    ':data-pane-id="pane.id"',
                                    ':data-tab-id="tab.id"',
                                ...
                                with vuetify.Template(v_if="tile && tile.variable_name"):
                                    html.Div(
                                        ...
                                        classes="pulsar-workspace-preview-title",
                                    )
                                    with html.Div(classes="pulsar-workspace-preview-media"):
                                        with vuetify.Template(v_if="tile.media_type === 'plot1d'"):
                                            html.Div(
                                                classes="pulsar-plot1d",
                                                raw_attrs=[
                                        ...
                                        with vuetify.Template(
                                            ...
                                            with vuetify.Template(
                                                ...
                                                html.Img(
                                                    src=("tile.src",),
                                                    raw_attrs=[
                                                        ':class="[\'pulsar-workspace-preview-image\', { \'pulsar-grid-image-sequence\': tile.media_type === \'image_sequence\' }]"',
                                            ...
                                            with vuetify.Template(
                                                ...
                                                html.Video(
                                                    src=("tile.src",),
                                                    class_="pulsar-workspace-preview-image pulsar-grid-video",
                                                    muted=True,
                                                    raw_attrs=[
                                                        'data-grid-video="1"',
                                                        ':data-fps="tile.fps || 2"',
                                ...
                                with vuetify.Template(v_if="!(tile && tile.variable_name)"):
                                    with html.Div(classes="pulsar-empty-cell"):
                                        html.Div("+", classes="pulsar-empty-plus")


def _build_workspace_splitters():
    html.Div(
        classes="pulsar-workspace-layout-surface",
        raw_attrs=[
    ...
    with vuetify.Template(v_for="pane in workspacePanes", key="'dock-' + pane.id"):
        with html.Div(
            classes="pulsar-workspace-tab-dock-preview",
            raw_attrs=[
    ...
    with vuetify.Template(v_for="split in workspaceSplitters", key="split.id"):
        with html.Div(
            classes="pulsar-workspace-splitter",
            raw_attrs=[
            ...
            html.Span(
                classes="pulsar-workspace-split-readout",
                raw_attrs=['aria-hidden="true"'],
            )
        ...
        with vuetify.VCol(
            ...
            with vuetify.VCard(
                ...
                with vuetify.VCardText(
                    classes="pulsar-workspace-card-content",
                    style=(
                        "height:100%;"
                        "min-height:0;"
                        "overflow:hidden;"
                    ...
                    html.Div(
                        "",
                        id="pulsar-reset-view-request",
                        style="display:none;",
                        raw_attrs=[
                            ':data-reset-view-request="JSON.stringify(resetViewRequest || {})"'
                        ],
                    )
                    with html.Div(classes="pulsar-vcr-bar pulsar-grid-controls-header"):
                        with html.Div(classes="pulsar-vcr-controls"):
                            html.Button(
                                "|<",
                                classes="pulsar-vcr-btn",
                                raw_attrs=[
                                    'type="button"',
                        ...
                        with html.Div(classes="pulsar-vcr-timeline"):
                            html.Span(
                                "Step = 0",
                                id="pulsar-vcr-time-value",
                                class_="text-caption pulsar-vcr-time",
                            )
                            html.Input(
                                type="range",
                                id="pulsar-vcr-step-slider",
                                classes="pulsar-vcr-slider",
                                change=(
                    ...
                    with vuetify.Template(v_if="scalarPlotStatus"):
                        html.Div(
                            "{{ scalarPlotStatus }}",
                            class_="text-caption pulsar-workspace-grid-status",
                            style="color:#8a4b00;",
                        )
                    _build_workspace_tab_bars(ctrl)
                    with html.Div(
                        classes="pulsar-main-grid pulsar-workspace-active-grid",
                        raw_attrs=[
                            ':key="workspaceActiveTabId"',
                            ":class=\"[workspacePanes.findIndex(pane => pane.id === workspaceActivePaneId) === 0 ? 'pulsar-workspace-slot-first' : 'pulsar-workspace-slot-second', { 'pulsar-freeform-canvas': gridLayoutMode === 'freeform', 'show-grid': gridLayoutMode === 'freeform' && canvasShowGrid }]\"",
                        ...
                        with vuetify.Template(
                            ...
                            with html.Div(classes="pulsar-canvas-empty-state"):
                                html.Div("Drop a variable anywhere", classes="pulsar-canvas-empty-title")
                                html.Div(
                                    "Tiles will snap to a {{ canvasCols }}-column canvas.",
                                    classes="pulsar-canvas-empty-copy",
                                )
                        html.Div(
                            classes="pulsar-canvas-placeholder",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        html.Div(
                            classes="pulsar-canvas-insertion-caret",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        html.Div(
                            classes="pulsar-canvas-guide pulsar-canvas-guide-v guide-v-1",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        html.Div(
                            classes="pulsar-canvas-guide pulsar-canvas-guide-v guide-v-2",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        html.Div(
                            classes="pulsar-canvas-guide pulsar-canvas-guide-h guide-h-1",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        html.Div(
                            classes="pulsar-canvas-guide pulsar-canvas-guide-h guide-h-2",
                            v_if="gridLayoutMode === 'freeform'",
                            raw_attrs=['aria-hidden="true"'],
                        )
                        with vuetify.Template(v_for="(tile, i) in gridCells", key="i"):
                            with html.Div(
                                ...
                                click=(
                                    ...
                                    "[i, (($event && $event.target && $event.target.closest && $event.target.closest('.pulsar-cell-close, .pulsar-timeline-driver-btn, .pulsar-scalar-field-ai-btn, .pulsar-grid-track-resize-handle')) ? 1 : 0), (($event && $event.shiftKey) ? 1 : 0)]",
                                ),
                                classes="pulsar-dropcell",
                                raw_attrs=[
                                    ':data-cell-index="i"',
                                    ':data-pane-id="workspaceActivePaneId"',
                                    ':data-tab-id="workspaceActiveTabId"',
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-col-resize-handle pulsar-grid-left-resize-handle",
                                    v_if=(
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-col-resize-handle",
                                    v_if=(
                                        "gridLayoutMode !== 'freeform'"
                                        " && !(gridLayoutMode === 'spanning' && tile && tile.grid_hidden)"
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-row-resize-handle pulsar-grid-top-resize-handle",
                                    v_if=(
                                        "gridLayoutMode !== 'freeform' && gridSizingMode !== 'fit'"
                                        " && !(gridLayoutMode === 'spanning' && tile && tile.grid_hidden)"
                                        " && ((gridLayoutMode === 'spanning'"
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-row-resize-handle",
                                    v_if=(
                                        "gridLayoutMode !== 'freeform'"
                                        " && !(gridLayoutMode === 'spanning' && tile && tile.grid_hidden)"
                                        " && (gridSizingMode !== 'fit' || ((gridLayoutMode === 'spanning'"
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-corner-resize-handle pulsar-grid-corner-bottom-left",
                                    v_if=(
                                        "gridLayoutMode !== 'freeform'"
                                        " && !(gridLayoutMode === 'spanning' && tile && tile.grid_hidden)"
                                        " && (gridSizingMode !== 'fit' || ("
                                ...
                                html.Div(
                                    classes="pulsar-grid-track-resize-handle pulsar-grid-corner-resize-handle pulsar-grid-corner-bottom-right",
                                    v_if=(
                                        "gridLayoutMode !== 'freeform'"
                                        " && !(gridLayoutMode === 'spanning' && tile && tile.grid_hidden)"
                                        " && (gridSizingMode !== 'fit' || ("
                                ...
                                with vuetify.Template(v_if="tile && tile.variable_name"):
                                    with html.Div(
                                        ...
                                        html.Button(
                                            ...
                                            classes="pulsar-scalar-field-ai-btn",
                                            click=(
                                        ...
                                        with html.Button(
                                            ...
                                            classes="pulsar-timeline-driver-btn",
                                            ...
                                            html.Span(
                                                classes="pulsar-timeline-clock-icon",
                                                raw_attrs=['aria-hidden="true"'],
                                            )
                                        html.Button(
                                            "x",
                                            classes="pulsar-cell-close",
                                    ...
                                    with html.Div(
                                        ...
                                        with vuetify.Template(v_if="tile.media_type === 'plot1d'"):
                                            ...
                                            with vuetify.Template(
                                                ...
                                                with html.Div(classes="pulsar-plot-legend"):
                                                    with vuetify.Template(
                                                        v_for="(item, j) in ((tile.plot && tile.plot.series) || [])",
                                                        key="j",
                                                    ):
                                                        with html.Div(
                                                            classes="pulsar-plot-legend-item",
                                                            raw_attrs=[
                                                                ':title="item.source_label || item.source_key || (\'Series \' + (j + 1))"',
                                                            ],
                                                        ):
                                                            html.Div(
                                                                classes="pulsar-plot-legend-line",
                                                                raw_attrs=[
                                        ...
                                        with vuetify.Template(v_if="tile.media_type !== 'plot1d'"):
                                            with vuetify.Template(v_if="tile.src"):
                                                with vuetify.Template(
                                                    ...
                                                    with html.Div(
                                                        classes="pulsar-scalar-field-view",
                                                        raw_attrs=[
                                                            ":style=\"{"
                                                            " '--pulsar-scalar-field-background': ((tile.scalar_field_settings && tile.scalar_field_settings.background_color) || '#000000'),"
                                                            " '--pulsar-scalar-field-foreground': ((tile.scalar_field_settings && tile.scalar_field_settings.foreground_color) || '#ffffff')"
                                                            " }\"",
                                                        ],
                                                    ):
                                                        with html.Div(
                                                            classes="pulsar-scalar-field-plot-frame pulsar-panzoom-viewport",
                                                            raw_attrs=[
                                                                ":class=\"{ 'pulsar-scalar-field-show-axes': tile.scalar_field_settings && tile.scalar_field_settings.show_axes }\"",
                                                            ],
                                                        ):
                                                            with html.Div(classes="pulsar-panzoom-content"):
                                                                with vuetify.Template(v_if="tile.media_type === 'image_sequence'"):
                                                                    html.Img(
                                                                        src=("tile.src",),
                                                                        class_="pulsar-grid-image-sequence",
                                                                        raw_attrs=[
                                                            ...
                                                            with vuetify.Template(
                                                                ...
                                                                with html.Div(
                                                                    classes="pulsar-scalar-field-y-axis",
                                                                    raw_attrs=[
                                                                        'data-scalar-axis="y"',
                                                                    ...
                                                                    with vuetify.Template(
                                                                        ...
                                                                        with html.Div(
                                                                            classes="pulsar-scalar-field-y-tick",
                                                                            raw_attrs=[
                                                                            ...
                                                                            html.Span(
                                                                                "{{ tick.label }}",
                                                                                classes="pulsar-scalar-field-tick-label",
                                                                            )
                                                                    html.Div(
                                                                        "{{ (tile.scalar_field_axes && tile.scalar_field_axes.y && tile.scalar_field_axes.y.label) || 'row' }}",
                                                                        classes="pulsar-scalar-field-y-label",
                                                                    )
                                                                with html.Div(
                                                                    classes="pulsar-scalar-field-x-axis",
                                                                    raw_attrs=[
                                                                        'data-scalar-axis="x"',
                                                                    ...
                                                                    with vuetify.Template(
                                                                        ...
                                                                        with html.Div(
                                                                            classes="pulsar-scalar-field-x-tick",
                                                                            raw_attrs=[
                                                                                ":class=\"{ 'is-start': tick.position === 0, 'is-end': tick.position === 100 }\"",
                                                                    ...
                                                                    html.Div(
                                                                        ...
                                                                        classes="pulsar-scalar-field-x-label",
                                                                    )
                                                        with vuetify.Template(
                                                            v_if=(
                                                                "tile.scalar_field_settings"
                                                            ...
                                                            with html.Div(classes="pulsar-scalar-field-colorbar"):
                                                                html.Div(
                                                                    classes="pulsar-scalar-field-colorbar-ramp",
                                                                    raw_attrs=[
                                                                        ":style=\"{ '--pulsar-scalar-field-colorbar': ((tile.scalar_field_settings && tile.scalar_field_settings.colorbar_gradient) || 'linear-gradient(to top, #440154, #fde725)') }\"",
                                                                    ],
                                                                )
                                                                with html.Div(classes="pulsar-scalar-field-colorbar-labels"):
                                                                    html.Div(
                                                                        "{{ tile.scalar_field_colorbar_max || tile.max || '' }}",
                                                                        classes="pulsar-scalar-field-colorbar-label",
                                                                        raw_attrs=[
                                                ...
                                                with vuetify.Template(
                                                    ...
                                                    with html.Div(classes="pulsar-panzoom-viewport"):
                                                        with html.Div(classes="pulsar-panzoom-content"):
                                                            html.Img(
                                                                src=("tile.src",),
                                                                class_="pulsar-grid-image-sequence",
                                ...
                                for resize_edge in (
                                    ...
                                    handle_classes = (
                                        "pulsar-canvas-resize-zone "
                                        f"is-{resize_edge}"
                                    )
                                    if resize_edge == "bottom-right":
                                        handle_classes += (
                                            " pulsar-canvas-resize-handle"
                                        )
                                    html.Div(
                                ...
                                with vuetify.Template(v_if="!(tile && tile.variable_name)"):
                                    with html.Div(classes="pulsar-empty-cell"):
                                        html.Div("+", classes="pulsar-empty-plus")
                                        html.Div("Drop variable here", classes="pulsar-empty-hover-label")
                    ...
                    with vuetify.Template(v_if="detailsSelectedVar"):
                        with html.Div(
                            ...
                            with vuetify.Template(v_if="detailsProvenanceChain"):
                                with html.Div(
                                    classes="pulsar-provenance-strip text-caption",
                                ):
                                    vuetify.VBtn(
                                        "Provenance",
                                        variant="tonal",
                                    ...
                                    html.Span(
                                        ...
                                        classes="pulsar-provenance-compact",
                                    )
                    with vuetify.Template(v_if="!detailsSelectedVar"):
